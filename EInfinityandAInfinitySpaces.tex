...
\begin{center}
\ \\
\scalebox{3}{\texttt{𝔼}_{\infty} \texttt{𝕒𝕟𝕕} \texttt{𝔸}_{\infty} \texttt{𝕊𝕡𝕒𝕔𝕖𝕤 𝕒𝕟𝕕 𝕥𝕙𝕖𝕚𝕣 𝔸𝕔𝕥𝕚𝕠𝕟𝕤}}\\
\ \\
\end{center}
\end{tcolorbox}
\end{center}
...
\begin{enumerate}
\item [-,A] and its commutative monoid structure
\item H spaces?
\item adjunctions involving H spaces
...
In this document I would like to explore derivations and flat connections.\\
...
\item https://en.wikipedia.org/wiki/Connection_(vector_bundle)
\item Lie algebra representations and ...
\item Here we tensor $\texttt{((ωⁱⁿᶠ.obj X).obj V) ∧ ???}$ with the Lie-algebra of derivations...\\
